...
First, we will conduct an exploratory analysis of our dataset, which comprises 18 sessions. Each session methodically documents the neural activity of mice in response to a variety of visual stimuli. We will carefully catalog each session's key aspects: the number of unique neurons, the total number of trials conducted, the diverse combinations of stimulus contrasts in the left and right visual fields, and the distribution of feedback types, classified as either success or failure. This detailed organization of our data provides a solid foundation for a thorough and nuanced analysis, enabling us to intricately examine and comprehend the patterns of neural activity elicited by the trials
